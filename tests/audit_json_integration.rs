// Test-file allows: pre-existing sanity asserts use `>= 0` on u32 line numbers (always true),
// one assert contains a known tautology, and one test name encodes a hex literal.
#![allow(
...
fn circular_dependencies_python_finds_cycle() {
    ...
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn cyclomatic_complexity_ts_finds_complex_function() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "cyclomatic_complexity" && f.pattern == "cyclomatic_complexity"),
        "expected cyclomatic_complexity finding; got: {:?}",
        findings
            .iter()
            .map(|f| (&f.pipeline, &f.pattern))
            .collect::<Vec<_>>()
    );
}
...
fn function_length_ts_finds_long_function() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "function_length" && f.pattern == "function_length"),
...
"#;
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "cognitive_complexity"
                && f.pattern == "high_cognitive_complexity"),
...
fn comment_to_code_ratio_ts_finds_under_documented() {
    ...
    assert!(
        findings.iter().any(
            |f| f.pipeline == "comment_to_code_ratio" && f.pattern == "comment_ratio_violation"
        ),
...
"#;
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "n_plus_one_queries" && f.pattern == "query_in_loop"),
...
fn sync_blocking_in_async_ts_finds_sync_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "sync_blocking_in_async" && f.pattern == "sync_call_in_async"),
...
fn race_conditions_rust_finds_static_mut() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "race_conditions" && f.pattern == "static_mut"),
...
fn type_confusion_rust_finds_union() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "type_confusion" && f.pattern == "union_type_confusion"),
...
fn unsafe_memory_rust_finds_unsafe_block() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "unsafe_memory" && f.pattern == "unsafe_memory_operation"),
...
fn integer_overflow_rust_finds_arithmetic() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "integer_overflow" && f.pattern == "unchecked_arithmetic"),
...
fn path_traversal_rust_finds_method_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "path_traversal" && f.pattern == "unvalidated_path_operation"),
...
fn resource_exhaustion_rust_finds_scoped_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "resource_exhaustion" && f.pattern == "unbounded_allocation"),
...
fn panic_dos_rust_finds_method_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "panic_dos" && f.pattern == "unwrap_untrusted"),
...
fn toctou_rust_finds_method_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "toctou" && f.pattern == "path_check_use_race"),
...
fn memory_leak_indicators_rust_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "memory_leak_indicators"),
...
fn command_injection_javascript_finds_exec_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "command_injection" && f.pattern == "exec_command_injection"),
...
fn code_injection_javascript_finds_direct_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "code_injection" && f.pattern == "code_injection_call"),
...
fn type_system_bypass_typescript_finds_as_expression() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "type_system_bypass" && f.pattern == "type_system_bypass"),
...
fn command_injection_go_finds_selector_call() {
    ...
    let graph = GraphBuilder::new(&workspace, &[Language::Go])
        .build()
        .unwrap();
...
fn command_injection_go_no_fp_fmt_println() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "command_injection" && f.file_path.ends_with(".go")),
        "expected no command_injection finding for fmt.Println; got: {:?}",
        findings
            .iter()
            .map(|f| (&f.pipeline, &f.pattern, &f.file_path))
            .collect::<Vec<_>>()
    );
}
...
fn go_path_traversal_go_finds_selector_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "go_path_traversal" && f.pattern == "unvalidated_path_join"),
...
fn race_conditions_go_finds_goroutine_in_loop() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "race_conditions" && f.pattern == "loop_var_capture"),
...
fn resource_exhaustion_go_finds_goroutine_in_loop() {
    ...
    assert!(
        findings.iter().any(
            |f| f.pipeline == "resource_exhaustion" && f.pattern == "unbounded_goroutine_spawn"
        ),
...
fn go_integer_overflow_go_finds_type_conversion() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "go_integer_overflow" && f.pattern == "narrowing_conversion"),
...
fn go_type_confusion_go_finds_type_assertion() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "go_type_confusion" && f.pattern == "unsafe_pointer_cast"),
...
fn command_injection_python_finds_attribute_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "command_injection" && f.pattern == "command_injection_call"),
...
fn command_injection_python_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "command_injection" && f.file_path.ends_with(".py")),
...
fn code_injection_python_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "code_injection" && f.file_path.ends_with(".py")),
...
fn path_traversal_python_finds_attribute_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "path_traversal" && f.pattern == "unvalidated_path_join"),
...
fn path_traversal_python_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "path_traversal" && f.file_path.ends_with(".py")),
...
fn insecure_deserialization_python_finds_attribute_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "insecure_deserialization"
                && f.pattern == "insecure_deserialization"),
...
fn insecure_deserialization_python_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "insecure_deserialization" && f.file_path.ends_with(".py")),
...
fn xxe_format_string_python_finds_attribute_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "xxe_format_string" && f.pattern == "xxe_format_string"),
...
fn xxe_format_string_python_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "xxe_format_string" && f.file_path.ends_with(".py")),
...
fn resource_exhaustion_python_finds_re_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "resource_exhaustion" && f.pattern == "redos_pattern"),
...
fn resource_exhaustion_python_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "resource_exhaustion" && f.file_path.ends_with(".py")),
...
fn memory_leak_indicators_python_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "memory_leak_indicators" && f.file_path.ends_with(".py")),
...
fn command_injection_java_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "command_injection" && f.file_path.ends_with(".java")),
...
fn weak_cryptography_java_finds_method_invocation() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "weak_cryptography" && f.pattern == "weak_crypto_usage"),
...
fn weak_cryptography_java_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "weak_cryptography" && f.file_path.ends_with(".java")),
...
fn insecure_deserialization_java_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "insecure_deserialization" && f.file_path.ends_with(".java")),
...
fn java_path_traversal_java_finds_object_creation() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "java_path_traversal"
                && f.pattern == "unvalidated_path_operation"),
...
fn java_path_traversal_java_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "java_path_traversal" && f.file_path.ends_with(".java")),
...
fn reflection_injection_java_finds_method_invocation() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "reflection_injection" && f.pattern == "reflection_injection"),
...
fn reflection_injection_java_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "reflection_injection" && f.file_path.ends_with(".java")),
...
fn java_race_conditions_java_finds_generic_field() {
    ...
    assert!(
        findings.iter().any(
            |f| f.pipeline == "java_race_conditions" && f.pattern == "thread_unsafe_collection"
        ),
...
fn java_race_conditions_java_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "java_race_conditions" && f.file_path.ends_with(".java")),
...
fn memory_leak_indicators_java_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "memory_leak_indicators" && f.file_path.ends_with(".java")),
...
fn format_string_c_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "format_string" && f.pattern == "format_string_vulnerability"),
...
fn format_string_c_clean() {
    ...
    std::fs::write(dir.path().join("test.c"), "struct Point { int x; int y; };").unwrap();
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "format_string" && f.file_path.ends_with(".c")),
...
fn c_command_injection_c_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "c_command_injection" && f.pattern == "command_injection_call"),
...
fn c_command_injection_c_clean() {
    ...
    std::fs::write(dir.path().join("test.c"), "typedef unsigned int uint32_t;").unwrap();
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "c_command_injection" && f.file_path.ends_with(".c")),
...
fn c_buffer_overflow_security_c_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "c_buffer_overflow_security"
                && f.pattern == "buffer_overflow_risk"),
...
fn c_buffer_overflow_security_c_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "c_buffer_overflow_security" && f.file_path.ends_with(".c")),
...
fn c_integer_overflow_c_finds_binary_expr() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "c_integer_overflow" && f.pattern == "unchecked_arithmetic"),
...
fn c_integer_overflow_c_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "c_integer_overflow" && f.file_path.ends_with(".c")),
...
fn c_toctou_c_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "c_toctou" && f.pattern == "toctou_check"),
...
fn c_toctou_c_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "c_toctou" && f.file_path.ends_with(".c")),
...
fn memory_leak_indicators_c_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "memory_leak_indicators" && f.file_path.ends_with(".c")),
...
fn c_weak_randomness_c_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "c_weak_randomness" && f.pattern == "weak_randomness"),
...
fn c_weak_randomness_c_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "c_weak_randomness" && f.file_path.ends_with(".c")),
...
fn c_memory_mismanagement_c_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "c_memory_mismanagement" && f.pattern == "memory_mismanagement"),
...
fn c_memory_mismanagement_c_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "c_memory_mismanagement" && f.file_path.ends_with(".c")),
...
fn c_path_traversal_c_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "c_path_traversal" && f.pattern == "path_traversal_risk"),
...
fn c_path_traversal_c_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "c_path_traversal" && f.file_path.ends_with(".c")),
...
fn c_uninitialized_memory_c_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "c_uninitialized_memory" && f.pattern == "uninitialized_memory"),
...
fn c_uninitialized_memory_c_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "c_uninitialized_memory" && f.file_path.ends_with(".c")),
...
fn cpp_injection_cpp_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "cpp_injection" && f.pattern == "command_injection_call"),
...
fn cpp_injection_cpp_clean() {
    ...
    std::fs::write(dir.path().join("test.cpp"), "class Foo { int x; int y; };").unwrap();
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "cpp_injection" && f.file_path.ends_with(".cpp")),
...
fn cpp_buffer_overflow_cpp_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "cpp_buffer_overflow" && f.pattern == "buffer_overflow_risk"),
...
fn cpp_buffer_overflow_cpp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "cpp_buffer_overflow" && f.file_path.ends_with(".cpp")),
...
fn cpp_exception_safety_cpp_finds_new() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "cpp_exception_safety" && f.pattern == "unguarded_allocation"),
...
fn cpp_exception_safety_cpp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "cpp_exception_safety" && f.file_path.ends_with(".cpp")),
...
fn cpp_memory_mismanagement_cpp_finds_delete() {
    ...
    assert!(
        findings.iter().any(
            |f| f.pipeline == "cpp_memory_mismanagement" && f.pattern == "memory_mismanagement"
        ),
...
fn cpp_memory_mismanagement_cpp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "cpp_memory_mismanagement" && f.file_path.ends_with(".cpp")),
...
fn cpp_race_conditions_cpp_finds_field() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "class Counter { int count; };").unwrap();
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "cpp_race_conditions" && f.pattern == "thread_unsafe_field"),
...
fn cpp_race_conditions_cpp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "cpp_race_conditions" && f.file_path.ends_with(".cpp")),
...
fn cpp_path_traversal_cpp_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "cpp_path_traversal" && f.pattern == "path_traversal_risk"),
...
fn cpp_path_traversal_cpp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "cpp_path_traversal" && f.file_path.ends_with(".cpp")),
...
fn memory_leak_indicators_cpp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "memory_leak_indicators" && f.file_path.ends_with(".cpp")),
        "expected no memory_leak_indicators finding for clean C++; got: {:?}",
        findings
            .iter()
            .map(|f| (&f.pipeline, &f.pattern, &f.file_path))
...
fn cpp_weak_randomness_cpp_finds_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "cpp_weak_randomness" && f.pattern == "weak_randomness"),
...
fn cpp_weak_randomness_cpp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "cpp_weak_randomness" && f.file_path.ends_with(".cpp")),
...
fn cpp_type_confusion_cpp_finds_new() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "cpp_type_confusion" && f.pattern == "type_confusion_cast"),
...
fn cpp_type_confusion_cpp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "cpp_type_confusion" && f.file_path.ends_with(".cpp")),
...
fn cpp_integer_overflow_cpp_finds_new() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "cpp_integer_overflow" && f.pattern == "unchecked_arithmetic"),
...
fn cpp_integer_overflow_cpp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "cpp_integer_overflow" && f.file_path.ends_with(".cpp")),
...
fn command_injection_csharp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "command_injection" && f.file_path.ends_with(".cs")),
...
fn weak_cryptography_csharp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "weak_cryptography" && f.file_path.ends_with(".cs")),
...
fn insecure_deserialization_csharp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "insecure_deserialization" && f.file_path.ends_with(".cs")),
...
fn csharp_path_traversal_csharp_finds_invocation() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "csharp_path_traversal" && f.pattern == "path_traversal_risk"),
...
fn csharp_path_traversal_csharp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "csharp_path_traversal" && f.file_path.ends_with(".cs")),
...
fn csharp_race_conditions_csharp_finds_field_decl() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "csharp_race_conditions" && f.pattern == "thread_unsafe_field"),
...
fn csharp_race_conditions_csharp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "csharp_race_conditions" && f.file_path.ends_with(".cs")),
...
fn reflection_unsafe_csharp_finds_invocation() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "reflection_unsafe" && f.pattern == "reflection_injection"),
...
fn reflection_unsafe_csharp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "reflection_unsafe" && f.file_path.ends_with(".cs")),
...
fn memory_leak_indicators_csharp_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "memory_leak_indicators" && f.file_path.ends_with(".cs")),
        "expected no memory_leak_indicators finding for clean C#; got: {:?}",
        findings
            .iter()
            .map(|f| (&f.pipeline, &f.pattern))
...
fn command_injection_php_finds_function_call() {
    ...
    let graph = GraphBuilder::new(&workspace, &[Language::Php])
        .build()
        .unwrap();
    let (findings, _) = AuditEngine::new()
...
fn command_injection_php_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "command_injection" && f.file_path.ends_with(".php")),
...
fn unsafe_include_php_finds_include_expression() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "unsafe_include" && f.pattern == "unsafe_include"),
...
fn unsafe_include_php_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "unsafe_include" && f.file_path.ends_with(".php")),
...
fn type_juggling_php_finds_binary_expression() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "type_juggling" && f.pattern == "loose_comparison"),
...
fn type_juggling_php_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "type_juggling" && f.file_path.ends_with(".php")),
...
fn unescaped_output_php_finds_echo_statement() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "unescaped_output" && f.pattern == "unescaped_output"),
...
fn unescaped_output_php_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "unescaped_output" && f.file_path.ends_with(".php")),
...
fn session_auth_php_finds_function_call() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "session_auth" && f.pattern == "session_management"),
...
fn session_auth_php_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "session_auth" && f.file_path.ends_with(".php")),
...
fn insecure_deserialization_php_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "insecure_deserialization" && f.file_path.ends_with(".php")),
...
fn memory_leak_indicators_php_clean() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "memory_leak_indicators" && f.file_path.ends_with(".php")),
...
fn panic_prone_calls_rust_finds_unwrap() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "panic_prone_calls_rust"),
...
fn panic_prone_calls_rust_finds_expect() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let x = Some(1).expect("msg"); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn panic_prone_calls_rust_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "panic_prone_calls_rust")
        .unwrap();
...
fn panic_prone_calls_rust_multiple_calls() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let a = Some(1).unwrap(); let b = Some(2).expect("x"); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "panic_prone_calls_rust")
        .count();
    assert!(
        count >= 2,
        "expected >= 2 panic_prone_calls_rust findings; got {count}"
    );
}
...
fn panic_prone_macros_rust_finds_panic_macro() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "panic_prone_macros_rust"),
...
fn clone_detection_rust_finds_clone_call() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let a = String::from("x"); let b = a.clone(); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "clone_detection"),
...
fn clone_detection_rust_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "clone_detection")
        .unwrap();
...
fn clone_detection_rust_multiple_clones() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let a = x.clone(); let b = y.clone(); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "clone_detection")
        .count();
    assert!(
        count >= 2,
        "expected >= 2 clone_detection findings; got {count}"
    );
}
...
fn clone_detection_rust_method_call_detected() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f(v: Vec<i32>) { let c = v.clone(); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn clone_detection_rust_ignores_iter_push() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let mut v: Vec<i32> = Vec::new(); v.push(1); let _ = v.iter(); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn god_object_detection_rust_finds_many_methods() {
    ...
    let methods: String = (0..10)
        .map(|i| format!("pub fn method_{i}(&self) {{}}\n"))
        .collect();
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "god_object_detection"),
...
fn god_object_detection_rust_no_findings_small_impl() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"struct Foo; impl Foo { fn a(&self){} fn b(&self){} fn c(&self){} }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn god_object_detection_rust_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "god_object_detection")
        .unwrap();
...
fn god_object_detection_rust_large_impl_detected() {
    ...
    let methods: String = (0..12)
        .map(|i| format!("pub fn method_{i}(&self) -> i32 {{ {i} }}\n"))
        .collect();
...
fn stringly_typed_rust_finds_ref_str_param() {
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "stringly_typed"),
...
fn stringly_typed_rust_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "stringly_typed")
        .unwrap();
...
fn stringly_typed_rust_multiple_ref_params() {
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "stringly_typed")
        .count();
    assert!(
        count >= 2,
        "expected >= 2 stringly_typed findings; got {count}"
    );
}
...
fn must_use_ignored_rust_finds_dropped_method_call() {
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "must_use_ignored"),
...
fn must_use_ignored_rust_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "must_use_ignored")
        .unwrap();
...
fn must_use_ignored_rust_multiple_dropped_calls() {
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "must_use_ignored")
        .count();
    assert!(count >= 2);
}
...
fn mutex_overuse_rust_finds_mutex_new() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"use std::sync::Mutex; fn f() { let m = Mutex::new(0); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "mutex_overuse"),
...
fn mutex_overuse_rust_finds_arc_new() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"use std::sync::Arc; fn f() { let a = Arc::new(42); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn mutex_overuse_rust_metadata_correct() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let m = std::sync::Mutex::new(0); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "mutex_overuse")
        .unwrap();
...
fn mutex_overuse_rust_rwlock_detected() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let rw = std::sync::RwLock::new(vec![]); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn pub_field_leakage_rust_finds_pub_field() {
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "pub_field_leakage"),
...
fn pub_field_leakage_rust_no_findings_private_fields() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"pub struct Foo { x: i32, y: String }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn pub_field_leakage_rust_multiple_pub_fields() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"pub struct Foo { pub a: i32, pub b: String, pub c: bool }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "pub_field_leakage")
        .count();
    assert!(
        count >= 3,
        "expected >= 3 pub_field_leakage findings; got {count}"
    );
}
...
fn pub_field_leakage_rust_metadata_correct() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"pub struct Leaky { pub host: String }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "pub_field_leakage")
        .unwrap();
...
fn pub_field_leakage_rust_internal_struct_pub_field() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"struct Internal { pub x: i32, pub y: String }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn missing_trait_abstraction_rust_finds_exported_fn() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "missing_trait_abstraction"),
...
fn missing_trait_abstraction_rust_finds_pub_fn() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"pub fn load(path: &str) -> Vec<u8> { vec![] }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn missing_trait_abstraction_rust_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "missing_trait_abstraction")
        .unwrap();
...
fn missing_trait_abstraction_rust_multiple_pub_fns() {
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "missing_trait_abstraction")
        .count();
    assert!(
        count >= 3,
        "expected >= 3 missing_trait_abstraction findings; got {count}"
    );
}
...
fn missing_trait_abstraction_rust_pub_method_detected() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"pub struct S; impl S { pub fn handle(&self) {} }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn async_blocking_rust_finds_scoped_call() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"async fn load() { let _ = std::fs::read("file.txt"); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "async_blocking"),
...
fn async_blocking_rust_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "async_blocking")
        .unwrap();
...
fn async_blocking_rust_multiple_scoped_calls() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let a = std::fs::read("a"); let b = std::fs::read("b"); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "async_blocking")
        .count();
    assert!(count >= 2);
}
...
fn async_blocking_rust_thread_sleep_detected() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { std::thread::sleep(std::time::Duration::from_secs(1)); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn async_blocking_rust_nested_path_detected() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let _ = std::net::TcpStream::connect("127.0.0.1:80"); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn async_blocking_rust_path_with_many_segments() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let _ = std::fs::OpenOptions::new(); }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn async_blocking_rust_no_findings_type_alias() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"type Result<T> = std::result::Result<T, String>;"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn async_blocking_rust_trait_method_scoped() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let _ = std::io::Write::write_all; }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn magic_numbers_rust_finds_integer_literal() {
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "magic_numbers"),
...
fn magic_numbers_rust_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "magic_numbers")
        .unwrap();
...
fn magic_numbers_rust_multiple_literals() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn f() { let a = 100; let b = 200; let c = 300; }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "magic_numbers")
        .count();
    assert!(
        count >= 3,
        "expected >= 3 magic_numbers findings; got {count}"
    );
}
...
fn dead_code_rust_finds_function() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"fn unused_helper() { println!("never called"); } fn main() {}"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn dead_code_rust_method_detected() {
    ...
    std::fs::write(
        dir.path().join("test.rs"),
        r#"struct Foo; impl Foo { fn helper(&self) {} }"#,
    )
    .unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Rust], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Rust])
...
fn duplicate_code_rust_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "duplicate_code")
        .unwrap();
...
fn coupling_rust_finds_use_declarations() {
    let dir = tempfile::tempdir().unwrap();
    let uses: String = (0..5)
        .map(|i| format!("use std::collections::HashMap{i};\n"))
        .collect();
...
"#,
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "error_swallowing"),
...
"#,
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "error_swallowing" && f.pattern == "swallowed_error"),
...
"#,
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "god_struct"),
...
fn god_struct_go_pattern_is_god_struct() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "god_struct" && f.pattern == "god_struct"),
...
"#,
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "god_struct")
        .count();
    assert!(
        count >= 2,
        "expected at least 2 god_struct findings for 2 structs; got {count}"
    );
}
...
"#,
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "goroutine_leak"),
...
"#,
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "goroutine_leak" && f.pattern == "goroutine_leak_risk"),
...
"#,
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "init_abuse"),
...
fn init_abuse_go_pattern_is_init_function_abuse() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "init_abuse" && f.pattern == "init_function_abuse"),
...
"#,
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "init_abuse")
        .count();
    assert!(
        count >= 3,
        "expected >= 3 init_abuse findings for 3 functions; got {count}"
    );
}
...
fn magic_numbers_go_pattern_is_magic_number() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "magic_numbers" && f.pattern == "magic_number"),
...
"#,
    ...
    assert!(
        count >= 2,
        "expected >= 2 magic_numbers findings; got {count}"
    );
}
...
"#,
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "mutex_misuse"),
...
"#,
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "mutex_misuse" && f.pattern == "mutex_misuse"),
...
"#,
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "naked_interface"),
...
fn naked_interface_go_pattern_is_naked_interface() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "naked_interface" && f.pattern == "naked_interface"),
...
"#,
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "stringly_typed_config"),
...
fn stringly_typed_config_go_pattern_is_stringly_typed_config() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "stringly_typed_config" && f.pattern == "stringly_typed_config"),
...
"#,
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "stringly_typed_config");
...
"#,
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "stringly_typed_config")
        .count();
    assert!(
        count >= 2,
        "expected >= 2 stringly_typed_config findings for 2 map params; got {count}"
    );
}
...
"#,
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "concrete_return_type"),
...
"#,
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "concrete_return_type" && f.pattern == "concrete_return_type"),
...
"#,
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "concrete_return_type");
...
"#,
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "context_not_propagated"),
...
"#,
    ...
    assert!(
        findings.iter().any(
            |f| f.pipeline == "context_not_propagated" && f.pattern == "context_not_propagated"
        ),
...
"#,
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "context_not_propagated");
...
fn dead_code_go_pattern_is_potentially_dead_export() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "dead_code" && f.pattern == "potentially_dead_export"),
...
"#,
    ...
    assert!(
        count >= 3,
        "expected >= 3 dead_code findings for 3 functions; got {count}"
    );
}
...
fn duplicate_code_go_pattern_is_potential_duplication() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "duplicate_code" && f.pattern == "potential_duplication"),
...
"#,
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "coupling" && f.pattern == "high_coupling"),
...
"#,
    ...
    assert!(
        count >= 1,
        "expected >= 1 coupling finding for single import; got {count}"
    );
}
...
fn bare_except_python_finds_bare_except() {
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "bare_except"),
...
fn bare_except_python_finds_multiple_bare_excepts() {
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "bare_except")
        .count();
    assert!(
        count >= 2,
        "expected >= 2 bare_except findings; got {count}"
    );
}
...
fn deep_nesting_python_finding_has_correct_pattern() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "deep_nesting" && f.pattern == "deep_nesting"),
...
fn deep_nesting_python_finding_severity_is_warning() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "deep_nesting" && f.severity == "warning"),
...
fn duplicate_logic_python_finds_functions() {
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "duplicate_logic"),
...
fn duplicate_logic_python_finding_has_correct_pattern() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.py"), "def foo(x):\n    return x\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "duplicate_logic" && f.pattern == "potential_duplication"),
...
fn empty_test_files_python_finds_test_file() {
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "empty_test_files"),
...
fn empty_test_files_python_finds_multiple_test_files() {
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "empty_test_files")
        .count();
    assert!(
        count >= 2,
        "expected >= 2 empty_test_files findings; got {count}"
    );
}
...
fn empty_test_files_python_finding_has_correct_pattern() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "empty_test_files" && f.pattern == "empty_test_file"),
...
fn empty_test_files_python_finding_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "empty_test_files" && f.severity == "info"),
...
fn god_functions_python_finds_large_function() {
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "god_functions"),
...
fn god_functions_python_finds_method_large() {
    let dir = tempfile::tempdir().unwrap();
    let body: String = (0..52)
        .map(|i| format!("        self.x{i} = {i}\n"))
        .collect();
...
fn god_functions_python_finding_has_correct_pattern() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "god_functions" && f.pattern == "god_function"),
...
fn god_functions_python_finding_severity_is_warning() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "god_functions" && f.severity == "warning"),
...
fn magic_numbers_python_finding_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "magic_numbers" && f.severity == "info"),
...
fn missing_type_hints_python_finds_exported_function() {
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "missing_type_hints"),
...
fn missing_type_hints_python_finds_multiple_functions() {
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "missing_type_hints")
        .count();
    assert!(
        count >= 2,
        "expected >= 2 missing_type_hints findings; got {count}"
    );
}
...
fn missing_type_hints_python_finding_has_correct_pattern() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "missing_type_hints" && f.pattern == "missing_type_hint"),
...
fn missing_type_hints_python_finding_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "missing_type_hints" && f.severity == "info"),
        "expected info severity"
    );
}
...
fn missing_type_hints_python_finds_class_level_functions() {
    ...
    assert!(
        count >= 2,
        "expected >= 2 missing_type_hints for class methods; got {count}"
    );
}
...
fn mutable_default_args_python_finds_list_default() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "mutable_default_args"),
...
fn mutable_default_args_python_finding_has_correct_pattern() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "mutable_default_args" && f.pattern == "mutable_default_arg"),
...
fn mutable_default_args_python_finding_severity_is_warning() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "mutable_default_args" && f.severity == "warning"),
        "expected warning severity"
    );
}
...
fn stringly_typed_python_finds_chained_comparisons() {
    ...
    assert!(
        count >= 3,
        "expected >= 3 stringly_typed findings for chained comparisons; got {count}"
    );
}
...
fn stringly_typed_python_finding_has_correct_pattern() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.py"), "if x == \"hello\":\n    pass\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "stringly_typed" && f.pattern == "stringly_typed"),
...
fn stringly_typed_python_finding_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "stringly_typed" && f.severity == "info"),
        "expected info severity"
    );
}
...
fn test_assertions_python_finds_assert_in_test_file() {
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "test_assertions"),
...
fn test_assertions_python_finds_multiple_asserts() {
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "test_assertions")
        .count();
    assert!(
        count >= 2,
        "expected >= 2 test_assertions findings; got {count}"
    );
}
...
fn test_assertions_python_finding_has_correct_pattern() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "test_assertions" && f.pattern == "weak_assertion"),
...
fn test_assertions_python_finding_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "test_assertions" && f.severity == "info"),
        "expected info severity"
    );
}
...
fn test_assertions_python_finds_multiple_in_one_test() {
    ...
    assert!(
        count >= 3,
        "expected >= 3 test_assertions findings; got {count}"
    );
}
...
fn test_hygiene_python_finds_decorator_in_test_file() {
    ...
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "test_hygiene"),
...
fn test_hygiene_python_finds_multiple_decorators() {
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "test_hygiene")
        .count();
    assert!(
        count >= 4,
        "expected >= 4 test_hygiene findings (one per decorator); got {count}"
    );
}
...
fn test_hygiene_python_finding_has_correct_pattern() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "test_hygiene" && f.pattern == "test_hygiene"),
...
fn test_hygiene_python_finding_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "test_hygiene" && f.severity == "info"),
        "expected info severity"
    );
}
...
fn test_hygiene_python_no_findings_non_test_file() {
    ...
    std::fs::write(dir.path().join("service.py"), "def api():\n    pass\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn test_pollution_python_finds_global_list() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test_example.py"), "SHARED_DATA = []\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
    let (findings, _) = AuditEngine::new()
        ...
        .unwrap();
    assert!(
        findings.iter().any(|f| f.pipeline == "test_pollution"),
...
fn test_pollution_python_finds_global_dict() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test_example.py"), "CACHE = {}\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn test_pollution_python_finds_module_level_assignment() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test_example.py"), "DATA = list()\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn test_pollution_python_finds_integer_constant() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test_example.py"), "TIMEOUT = 30\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn test_pollution_python_finds_multiple_module_assignments() {
    ...
    let count = findings
        .iter()
        .filter(|f| f.pipeline == "test_pollution")
        .count();
    assert!(
        count >= 3,
        "expected >= 3 test_pollution findings; got {count}"
    );
}
...
fn test_pollution_python_finding_has_correct_pattern() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test_example.py"), "SHARED = []\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "test_pollution" && f.pattern == "test_pollution"),
...
fn test_pollution_python_finding_severity_is_warning() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "test_pollution" && f.severity == "warning"),
        "expected warning severity"
    );
}
...
fn test_pollution_python_finds_in_tests_subdir() {
    ...
    let test_dir = dir.path().join("tests");
    std::fs::create_dir(&test_dir).unwrap();
    std::fs::write(test_dir.join("test_api.py"), "QUEUE = []\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn test_pollution_python_finds_augmented_assignment() {
    ...
    std::fs::write(dir.path().join("test_example.py"), "counter = 0\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn test_pollution_python_finds_deque_assignment() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test_example.py"), "QUEUE = deque()\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn test_pollution_python_finds_none_assignment() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test_example.py"), "db = None\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn test_pollution_python_no_findings_no_assignment_in_test() {
    ...
    std::fs::write(dir.path().join("test_example.py"), "pass\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn dead_code_python_finds_function_symbol() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("service.py"), "def process():\n    pass\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn dead_code_python_finding_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "dead_code" && f.severity == "info"),
        "expected info severity"
    );
}
...
fn dead_code_python_no_findings_variable_only() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("service.py"), "x = 1\ny = 2\nz = x + y\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn duplicate_code_python_finding_has_correct_pattern() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("service.py"), "def foo():\n    pass\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn coupling_python_finding_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "coupling" && f.severity == "info"),
        "expected info severity"
    );
}
...
fn coupling_python_finds_mixed_import_types() {
    ...
    assert!(
        count >= 3,
        "expected >= 3 coupling findings for mixed imports; got {count}"
    );
}
...
fn coupling_python_finds_star_import() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("service.py"), "from os import *\n").unwrap();
    let workspace = Workspace::load(dir.path(), &[Language::Python], Some(10_000_000)).unwrap();
    let graph = GraphBuilder::new(&workspace, &[Language::Python])
        .build()
        .unwrap();
...
fn deprecated_mysql_api_php_finds_mysql_connect() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "deprecated_mysql_api"),
...
fn deprecated_mysql_api_php_finds_multiple_calls() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "deprecated_mysql_api")
            .count()
            >= 2,
...
fn deprecated_mysql_api_php_clean_empty_file() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "deprecated_mysql_api" && f.file_path.ends_with(".php")),
...
fn error_suppression_php_finds_multiple_suppressions() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "error_suppression")
            .count()
            >= 2,
...
fn error_suppression_php_clean_no_at_operator() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "error_suppression" && f.pattern == "error_suppression"),
...
fn extract_usage_php_clean_no_extract() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "extract_usage" && f.pattern == "extract_usage"),
...
fn extract_usage_php_clean_empty_file() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "extract_usage" && f.file_path.ends_with(".php")),
...
fn extract_usage_php_clean_compact_not_extract() {
    ...
    let extract_findings: Vec<_> = findings
        .iter()
...
fn god_class_php_finds_any_class() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "god_class" && f.pattern == "god_class"),
...
fn logic_in_views_php_finds_nested_if() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "logic_in_views")
            .count()
            >= 2,
...
fn missing_type_declarations_php_finds_untyped_function() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "missing_type_declarations"),
...
fn missing_type_declarations_php_finds_multiple_functions() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "missing_type_declarations")
            .count()
            >= 2,
...
fn silent_exception_php_finds_multiple_catches() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "silent_exception")
            .count()
            >= 2,
...
fn exception_swallowing_java_finds_catch_clause() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "exception_swallowing"),
...
fn exception_swallowing_java_finds_multiple_catches() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "exception_swallowing")
            .count()
            >= 2,
...
fn exception_swallowing_java_pattern_is_empty_catch() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "exception_swallowing" && f.pattern == "empty_catch"),
...
fn exception_swallowing_java_severity_is_warning() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "exception_swallowing" && f.severity == "warning")
    );
}
...
fn god_class_java_finds_class_symbol() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(
        dir.path().join("Big.java"),
        "class BigClass { void m1(){} void m2(){} }",
    )
    .unwrap();
    let findings = run_java_tech_debt(&dir);
    assert!(
...
fn god_class_java_severity_is_warning() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "god_class" && f.severity == "warning")
    );
}
...
fn god_class_java_finds_class_with_methods() {
    ...
    let methods: String = (0..12)
        ...
        .join(" ");
    std::fs::write(
        dir.path().join("Large.java"),
        format!("class Large {{ {} }}", methods),
    )
    .unwrap();
    let findings = run_java_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "god_class"));
...
fn god_class_java_finds_class_with_fields() {
    ...
    let fields: String = (0..15)
        ...
        .join(" ");
    std::fs::write(
        dir.path().join("DataObj.java"),
        format!("class DataObj {{ {} }}", fields),
    )
    .unwrap();
    let findings = run_java_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "god_class"));
...
fn god_class_java_excludes_test_files() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(
        dir.path().join("MyTest.java"),
        "class MyTest { void testSomething(){} }",
    )
    .unwrap();
    let findings = run_java_tech_debt(&dir);
    assert!(
...
fn god_class_java_finds_public_class() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(
        dir.path().join("Pub.java"),
        "public class Pub { void doWork(){} }",
    )
    .unwrap();
    let findings = run_java_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "god_class"));
...
fn instanceof_chains_java_pattern_is_instanceof_chain() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "instanceof_chains" && f.pattern == "instanceof_chain")
    );
}
...
fn instanceof_chains_java_severity_is_warning() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "instanceof_chains" && f.severity == "warning")
    );
}
...
fn instanceof_chains_java_finds_multiple_instanceof() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "instanceof_chains")
            .count()
            >= 3,
...
fn magic_strings_java_pattern_is_magic_string() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "magic_strings" && f.pattern == "magic_string")
    );
}
...
fn magic_strings_java_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "magic_strings" && f.severity == "info")
    );
}
...
fn magic_strings_java_finds_multiple_string_args() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "magic_strings")
            .count()
            >= 2
    );
}
...
fn missing_final_java_pattern_is_missing_final_field() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "missing_final" && f.pattern == "missing_final_field")
    );
}
...
fn missing_final_java_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "missing_final" && f.severity == "info")
    );
}
...
fn missing_final_java_finds_multiple_fields() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "missing_final")
            .count()
            >= 3
    );
}
...
fn missing_final_java_finds_final_field_too() {
    ...
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(
        dir.path().join("Const.java"),
        "class Const { private final int MAX = 10; }",
    )
    .unwrap();
    let findings = run_java_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "missing_final"));
...
fn mutable_public_fields_java_finds_exported_variable() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "mutable_public_fields"),
...
fn mutable_public_fields_java_clean_no_exported_variables() {
    ...
    std::fs::write(
        dir.path().join("Svc.java"),
        "class Svc { private int count; void m(){} }",
    )
    .unwrap();
    let findings = run_java_tech_debt(&dir);
    assert!(
...
fn mutable_public_fields_java_pattern_is_mutable_public_field() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "mutable_public_fields" && f.pattern == "mutable_public_field")
    );
}
...
fn mutable_public_fields_java_severity_is_warning() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "mutable_public_fields" && f.severity == "warning")
    );
}
...
fn mutable_public_fields_java_finds_multiple_public_fields() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "mutable_public_fields")
            .count()
            >= 3
    );
}
...
fn mutable_public_fields_java_excludes_test_files() {
    ...
    std::fs::write(
        ...
        "class FooTest { public String field; }",
    )
    .unwrap();
    let findings = run_java_tech_debt(&dir);
    assert!(
...
fn null_returns_java_pattern_is_null_return() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "null_returns" && f.pattern == "null_return")
    );
}
...
fn null_returns_java_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "null_returns" && f.severity == "info")
    );
}
...
fn null_returns_java_finds_multiple_null_returns() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "null_returns")
            .count()
            >= 3
    );
}
...
fn raw_types_java_pattern_is_raw_generic_type() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "raw_types" && f.pattern == "raw_generic_type")
    );
}
...
fn raw_types_java_severity_is_warning() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "raw_types" && f.severity == "warning")
    );
}
...
fn resource_leaks_java_pattern_is_resource_leak() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "resource_leaks" && f.pattern == "resource_leak")
    );
}
...
fn resource_leaks_java_severity_is_warning() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "resource_leaks" && f.severity == "warning")
    );
}
...
fn resource_leaks_java_finds_multiple_creations() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "resource_leaks")
            .count()
            >= 2
    );
}
...
fn static_utility_sprawl_java_finds_class_symbol() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "static_utility_sprawl"),
...
fn static_utility_sprawl_java_pattern_is_static_utility_class() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "static_utility_sprawl" && f.pattern == "static_utility_class")
    );
}
...
fn static_utility_sprawl_java_severity_is_info() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "static_utility_sprawl" && f.severity == "info")
    );
}
...
fn static_utility_sprawl_java_excludes_test_files() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(
        dir.path().join("UtilsTest.java"),
        "class UtilsTest { void testA(){} }",
    )
    .unwrap();
    let findings = run_java_tech_debt(&dir);
    assert!(
...
fn static_utility_sprawl_java_finds_multiple_classes() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "static_utility_sprawl")
            .count()
            >= 2
    );
}
...
fn string_concat_in_loops_java_finds_assignment_expression() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "string_concat_in_loops"),
...
fn string_concat_in_loops_java_severity_is_warning() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "string_concat_in_loops" && f.severity == "warning")
    );
}
...
fn string_concat_in_loops_java_finds_multiple_assignments() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "string_concat_in_loops")
            .count()
            >= 2
    );
}
...
fn coupling_java_pattern_is_excessive_imports() {
    ...
    for f in &coupling_findings {
        assert_eq!(
            f.pattern, "high_coupling",
            "coupling pipeline should use pattern 'high_coupling', got '{}'",
            f.pattern
        );
    }
}
...
fn buffer_overflows_c_finds_strcpy_call() {
    ...
    assert!(
        findings.iter().any(|f| f.pipeline == "buffer_overflows"),
        "expected buffer_overflows finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn buffer_overflows_c_finds_strcat_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(char *d, char *s) { strcat(d, s); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
...
fn buffer_overflows_c_finds_sprintf_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        r#"void f(char *buf, char *name) { sprintf(buf, "%s", name); }"#,
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "buffer_overflows"));
...
fn buffer_overflows_c_finds_vsprintf_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(char *buf, va_list args) { vsprintf(buf, \"%s\", args); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "buffer_overflows"));
...
fn buffer_overflows_c_finds_wcscpy_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(wchar_t *d, wchar_t *s) { wcscpy(d, s); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "buffer_overflows"));
...
fn buffer_overflows_c_finds_multiple_unsafe_calls() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(char *d, char *s, char *buf) { strcpy(d, s); sprintf(buf, \"%s\", s); gets(buf); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "buffer_overflows")
            .count()
            >= 2
    );
}
...
fn buffer_overflows_c_finds_stpcpy_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(char *d, char *s) { stpcpy(d, s); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "buffer_overflows"));
...
fn buffer_overflows_c_finds_sscanf_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(char *buf) { int x; sscanf(buf, \"%d\", &x); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "buffer_overflows"));
...
fn define_instead_of_inline_c_finds_simple_macro() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "define_instead_of_inline"),
        "expected define_instead_of_inline finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn define_instead_of_inline_c_finds_multiple_macros() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "define_instead_of_inline")
            .count()
            >= 2
    );
}
...
fn global_mutable_state_c_finds_int_global() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "global_mutable_state"),
        "expected global_mutable_state finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn global_mutable_state_c_finds_multiple_globals() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "global_mutable_state")
            .count()
            >= 2
    );
}
...
fn ignored_return_values_c_finds_fopen_ignored() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "ignored_return_values"),
        "expected ignored_return_values finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn ignored_return_values_c_finds_fwrite_ignored() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(FILE *fp) { fwrite(buf, 1, 10, fp); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn ignored_return_values_c_finds_fread_ignored() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(FILE *fp) { fread(buf, 1, 10, fp); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn ignored_return_values_c_finds_send_ignored() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(int sock) { send(sock, buf, 10, 0); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn ignored_return_values_c_finds_multiple_ignored() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(FILE *fp, int sock) { fwrite(buf, 1, 10, fp); send(sock, buf, 10, 0); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "ignored_return_values")
            .count()
            >= 2
    );
}
...
fn ignored_return_values_c_finds_recv_ignored() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(int sock) { recv(sock, buf, 100, 0); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn ignored_return_values_c_finds_snprintf_ignored() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(char *buf) { snprintf(buf, 10, \"%s\", \"hi\"); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn magic_numbers_c_finds_multiple_literals() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f() { int x = 42; int y = 99; int z = 777; }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn memory_leaks_c_finds_malloc_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f() { int *p = malloc(10); p[0] = 1; }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "memory_leaks"),
        "expected memory_leaks finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn memory_leaks_c_finds_calloc_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f() { int *p = calloc(10, sizeof(int)); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
...
fn memory_leaks_c_finds_strdup_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(char *s) { char *copy = strdup(s); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "memory_leaks"));
...
fn memory_leaks_c_finds_aligned_alloc_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f() { void *p = aligned_alloc(16, 64); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "memory_leaks"));
...
fn memory_leaks_c_finds_multiple_allocs() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f() { int *p = malloc(10); int *q = calloc(5, 4); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "memory_leaks")
            .count()
            >= 2
    );
}
...
fn memory_leaks_c_finds_strndup_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(char *s) { char *t = strndup(s, 10); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "memory_leaks"));
...
fn memory_leaks_c_finds_mmap_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f() { void *p = mmap(0, 4096, 3, 1, -1, 0); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "memory_leaks"));
...
fn missing_const_c_finds_read_only_pointer_param() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "int sum(int *arr, int n) { return arr[0]; }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "missing_const"),
        "expected missing_const finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn missing_const_c_finds_struct_pointer_param() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "struct Foo { int x; };\nvoid print_foo(struct Foo *f) { int v = f->x; }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "missing_const"));
...
fn missing_const_c_finds_void_pointer_param() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "int check(void *data) { return data != 0; }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "missing_const"));
...
fn missing_const_c_finds_multiple_pointer_params() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(int *a, char *b) { int x = *a; int y = b[0]; }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "missing_const")
            .count()
            >= 2
    );
}
...
fn missing_const_c_finds_double_ptr_in_different_fn() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "int get_val(double *d) { return (int)*d; }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "missing_const"));
...
fn missing_const_c_finds_unsigned_ptr_param() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "int sum_u(unsigned int *arr, int n) { return arr[0]; }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "missing_const"));
...
fn raw_struct_serialization_c_finds_fwrite_with_sizeof() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "raw_struct_serialization"),
        "expected raw_struct_serialization finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn raw_struct_serialization_c_finds_write_syscall_with_sizeof() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "struct M { int t; };\nvoid f(int fd) { struct M m; write(fd, &m, sizeof(struct M)); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn raw_struct_serialization_c_finds_multiple_calls() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "raw_struct_serialization")
            .count()
            >= 2
    );
}
...
fn raw_struct_serialization_c_finds_sizeof_in_args() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(FILE *fp, int *buf) { fwrite(buf, sizeof(int), 10, fp); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn signed_unsigned_mismatch_c_finds_int_loop() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "signed_unsigned_mismatch"),
        "expected signed_unsigned_mismatch finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn signed_unsigned_mismatch_c_finds_int_vs_strlen() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(char *s) { for (int i = 0; i < strlen(s); i++) {} }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn signed_unsigned_mismatch_c_finds_multiple_loops() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "signed_unsigned_mismatch")
            .count()
            >= 2
    );
}
...
fn signed_unsigned_mismatch_c_finds_short_counter() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(short n) { for (short i = 0; i < n; i++) {} }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn signed_unsigned_mismatch_c_finds_nested_loops() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(int m, int n) { for (int i = 0; i < m; i++) { for (int j = 0; j < n; j++) {} } }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn signed_unsigned_mismatch_c_finds_int_count_loop() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f(int count) { for (int idx = 0; idx < count; idx++) {} }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(
...
fn typedef_pointer_hiding_c_finds_int_ptr_typedef() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "typedef_pointer_hiding"),
        "expected typedef_pointer_hiding finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn typedef_pointer_hiding_c_finds_multiple_ptr_typedefs() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "typedef_pointer_hiding")
            .count()
            >= 2
    );
}
...
fn unchecked_malloc_c_finds_malloc_call() {
    ...
    let findings = run_c_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "unchecked_malloc"),
        "expected unchecked_malloc finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn unchecked_malloc_c_finds_aligned_alloc_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f() { void *p = aligned_alloc(16, 1024); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "unchecked_malloc"));
...
fn unchecked_malloc_c_finds_multiple_alloc_calls() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "unchecked_malloc")
            .count()
            >= 2
    );
}
...
fn unchecked_malloc_c_finds_posix_memalign_call() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void f() { void *p; posix_memalign(&p, 16, 64); }",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "unchecked_malloc"));
...
fn void_pointer_abuse_c_finds_void_ptr_param() {
    ...
    let findings = run_c_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "void_pointer_abuse"),
        "expected void_pointer_abuse finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn void_pointer_abuse_c_finds_void_ptr_in_callback() {
    ...
    std::fs::write(
        dir.path().join("test.c"),
        "void register_handler(void *ctx, int event) {}",
    )
    .unwrap();
    let findings = run_c_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "void_pointer_abuse"));
...
fn void_pointer_abuse_c_finds_multiple_ptr_params() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "void_pointer_abuse")
            .count()
            >= 2
    );
}
...
fn c_style_cast_cpp_finds_int_cast() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "void f() { int x = (int)3.14; }",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "c_style_cast"),
        "expected c_style_cast finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn c_style_cast_cpp_finds_multiple_casts() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "void f() { int x = (int)3.14; char c = (char)65; }",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "c_style_cast")
            .count()
            >= 2
    );
}
...
fn c_style_cast_cpp_clean_no_cast() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "void f() { int x = static_cast<int>(3.14); }",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "c_style_cast"),
        "expected no c_style_cast finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn c_style_cast_cpp_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "c_style_cast")
        .unwrap();
    assert!(!f.pattern.is_empty());
...
fn endl_flush_cpp_finds_std_endl() {
    ...
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "endl_flush"),
        "expected endl_flush finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn endl_flush_cpp_finds_multiple_endl() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "void f() { std::cout << std::endl; std::cout << std::endl; }",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "endl_flush")
            .count()
            >= 2
    );
}
...
fn endl_flush_cpp_finds_std_string_qualified_id() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "void f() { std::string s; std::cout << std::endl; }",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
...
fn endl_flush_cpp_clean_newline_char() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "void f() { int x = 42; return; }",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "endl_flush"),
        "expected no endl_flush finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn endl_flush_cpp_clean_no_endl() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void f() { int x = 42; }").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(!findings.iter().any(|f| f.pipeline == "endl_flush"));
}
...
fn exception_across_boundary_cpp_finds_throw_stmt() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "exception_across_boundary"),
        "expected exception_across_boundary finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn exception_across_boundary_cpp_finds_nested_throw() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "void f() { if (true) { throw std::runtime_error(\"err\"); } }",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
...
fn exception_across_boundary_cpp_finds_multiple_throws() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "exception_across_boundary")
            .count()
            >= 2
    );
}
...
fn exception_across_boundary_cpp_clean_no_throw() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "exception_across_boundary"),
        "expected no exception_across_boundary finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn exception_across_boundary_cpp_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "exception_across_boundary")
        .unwrap();
    assert!(!f.pattern.is_empty());
    assert!(f.file_path.ends_with(".cpp"));
}
...
fn excessive_includes_cpp_finds_single_include() {
    ...
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "excessive_includes"),
        "expected excessive_includes finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn excessive_includes_cpp_finds_multiple_includes() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "excessive_includes")
            .count()
            >= 5
    );
}
...
fn excessive_includes_cpp_clean_no_includes() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "excessive_includes"),
        "expected no excessive_includes finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn excessive_includes_cpp_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "excessive_includes")
        .unwrap();
...
fn large_object_by_value_cpp_finds_string_param() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void f(std::string s) {}").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "large_object_by_value"),
        "expected large_object_by_value finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn large_object_by_value_cpp_finds_vector_param() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void f(std::vector<int> v) {}").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn large_object_by_value_cpp_finds_int_param() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void f(int x) {}").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn large_object_by_value_cpp_finds_multiple_params() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "large_object_by_value")
            .count()
            >= 3
    );
}
...
fn large_object_by_value_cpp_finds_map_param() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "void f(std::map<std::string, int> data) {}",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
...
fn large_object_by_value_cpp_finds_deque_param() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void f(std::deque<int> d) {}").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn large_object_by_value_cpp_finds_set_param() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void f(std::set<int> s) {}").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn large_object_by_value_cpp_clean_no_params() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "large_object_by_value"),
        "expected no large_object_by_value finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn large_object_by_value_cpp_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "large_object_by_value")
        .unwrap();
...
fn magic_numbers_cpp_finds_float_literal() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void f() { float x = 3.14; }").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "magic_numbers"));
}
...
fn magic_numbers_cpp_finds_buffer_size_literal() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void f() { char buf[1024]; }").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "magic_numbers"));
}
...
fn magic_numbers_cpp_finds_literal_in_return() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "int f() { return 404; }").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "magic_numbers"));
}
...
fn magic_numbers_cpp_finds_multiple_literals() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "void f() { int x = 42; int y = 99; int z = 7; }",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
...
fn magic_numbers_cpp_finds_hex_literal() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void f() { int x = 0xDEAD; }").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "magic_numbers"));
}
...
fn magic_numbers_cpp_clean_no_literals() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "magic_numbers"),
        "expected no magic_numbers finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn magic_numbers_cpp_finds_large_literal() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void f() { int x = 65535; }").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "magic_numbers"));
}
...
fn missing_override_cpp_finds_method_in_class() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "class Foo { public: int doIt() { return 42; } };",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "missing_override"),
        "expected missing_override finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
"#,
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "missing_override")
            .count()
            >= 2
    );
}
...
fn missing_override_cpp_clean_no_methods() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "void standalone() { int x = 0; }",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "missing_override"),
        "expected no missing_override finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn missing_override_cpp_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "missing_override")
        .unwrap();
...
fn raw_memory_management_cpp_finds_raw_new() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "raw_memory_management"),
        "expected raw_memory_management finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn raw_memory_management_cpp_finds_raw_delete() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void f(int* p) { delete p; }").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
"#,
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "raw_memory_management")
            .count()
            >= 2
    );
}
...
fn raw_memory_management_cpp_clean_make_unique() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "void f() { auto p = std::make_unique<int>(42); }",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "raw_memory_management"),
        "expected no raw_memory_management finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn raw_memory_management_cpp_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "raw_memory_management")
        .unwrap();
...
fn raw_union_cpp_finds_named_union() {
    ...
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "raw_union"),
        "expected raw_union finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn raw_union_cpp_finds_union_with_pod_members() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "union Reg { unsigned int raw; struct { unsigned short lo; unsigned short hi; } parts; };",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "raw_union"));
...
fn raw_union_cpp_clean_no_union() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "raw_union"),
        "expected no raw_union finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
"#,
    ...
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "rule_of_five"),
        "expected rule_of_five finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
"#,
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "rule_of_five")
            .count()
            >= 2
    );
}
...
fn rule_of_five_cpp_clean_no_classes() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "rule_of_five"),
        "expected no rule_of_five finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn rule_of_five_cpp_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "class Foo { int x; };").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "rule_of_five")
        .unwrap();
...
"#,
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "shared_ptr_cycle_risk"),
        "expected shared_ptr_cycle_risk finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
"#,
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "shared_ptr_cycle_risk")
            .count()
            >= 2
    );
}
...
fn shared_ptr_cycle_risk_cpp_finds_pointer_field() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "class Foo { int* ptr; };").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn shared_ptr_cycle_risk_cpp_clean_no_fields() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "shared_ptr_cycle_risk"),
        "expected no shared_ptr_cycle_risk finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn shared_ptr_cycle_risk_cpp_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "shared_ptr_cycle_risk")
        .unwrap();
...
fn uninitialized_member_cpp_finds_int_field() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "uninitialized_member"),
        "expected uninitialized_member finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn uninitialized_member_cpp_finds_float_field() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "struct Bar { float val; };").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn uninitialized_member_cpp_finds_multiple_fields() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "uninitialized_member")
            .count()
            >= 3
    );
}
...
fn uninitialized_member_cpp_finds_bool_field() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "class Foo { bool flag; };").unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn uninitialized_member_cpp_clean_no_fields() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "uninitialized_member"),
        "expected no uninitialized_member finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn uninitialized_member_cpp_finds_char_field() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "class Foo { char c; unsigned int u; };",
    )
    .unwrap();
    let findings = run_cpp_tech_debt(&dir);
    assert!(
...
fn uninitialized_member_cpp_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "uninitialized_member")
        .unwrap();
...
fn dead_code_cpp_finds_function_with_body() {
    ...
    std::fs::write(
        dir.path().join("test.cpp"),
        "static void compute() { int x = 1; int y = 2; int z = x + y; }",
    )
    .unwrap();
    let findings = run_cpp_code_style(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "dead_code"));
...
fn dead_code_cpp_clean_no_functions() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "dead_code"),
        "expected no dead_code finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
"#,
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "duplicate_code")
            .count()
            >= 3
    );
}
...
fn duplicate_code_cpp_clean_no_functions() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "duplicate_code"),
        "expected no duplicate_code finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn duplicate_code_cpp_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cpp"), "void helper() { int x = 1; }").unwrap();
    let findings = run_cpp_code_style(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "duplicate_code")
...
fn coupling_cpp_clean_no_includes() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "coupling"),
        "expected no coupling finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn argument_mutation_js_finds_member_assignment() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "function foo(obj) { obj.name = 'bar'; }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "argument_mutation"),
        "expected argument_mutation finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn argument_mutation_js_finds_nested_member() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "function f(config) { config.nested.deep = true; }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "argument_mutation"));
...
fn argument_mutation_js_finds_multiple_mutations() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "argument_mutation")
            .count()
            >= 2
    );
}
...
fn argument_mutation_js_finds_method_body_mutation() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "class C { process(data) { data.value = 42; } }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "argument_mutation"));
...
fn argument_mutation_js_finds_obj_prop_reassign() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "function update(state) { state.loading = true; state.error = null; }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
...
fn argument_mutation_js_clean_local_variable() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "function f(obj) { let local = {}; local.name = 'bar'; }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
...
fn argument_mutation_js_clean_no_assignments() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "argument_mutation"),
        "expected no argument_mutation finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn argument_mutation_js_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "function f(obj) { obj.x = 1; }").unwrap();
    let findings = run_js_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "argument_mutation")
        .unwrap();
...
fn argument_mutation_js_finds_conditional_mutation() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "function f(opts) { if (opts.debug) { opts.verbose = true; } }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "argument_mutation"));
...
fn callback_hell_js_finds_arrow_callback() {
    ...
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "callback_hell"),
        "expected callback_hell finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn callback_hell_js_finds_mixed_callbacks() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "callback_hell")
            .count()
            >= 3
    );
}
...
fn callback_hell_js_clean_no_callbacks() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "callback_hell"),
        "expected no callback_hell finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn callback_hell_js_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "a(() => {});").unwrap();
    let findings = run_js_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "callback_hell")
        .unwrap();
...
fn console_log_in_prod_js_finds_console_log() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "console.log('hello');").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "console_log"),
        "expected console_log finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn console_log_in_prod_js_finds_console_warn() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "console.warn('warning');").unwrap();
    let findings = run_js_tech_debt(&dir);
...
fn console_log_in_prod_js_finds_console_error() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "console.error('err');").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "console_log"));
}
...
fn console_log_in_prod_js_finds_console_debug() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "console.debug('debug');").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "console_log"));
}
...
fn console_log_in_prod_js_finds_console_info() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "console.info('info');").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "console_log"));
}
...
fn console_log_in_prod_js_finds_multiple_calls() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "console_log")
            .count()
            >= 3
    );
}
...
fn console_log_in_prod_js_clean_no_console() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "console_log"),
        "expected no console_log finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn console_log_in_prod_js_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "console.log('x');").unwrap();
    let findings = run_js_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "console_log")
        .unwrap();
...
fn event_listener_leak_js_finds_add_event_listener() {
    ...
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "event_listener_leak"),
        "expected event_listener_leak finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn event_listener_leak_js_finds_multiple_listeners() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "el.addEventListener('click', h1); el.addEventListener('keyup', h2);",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "event_listener_leak")
            .count()
            >= 2
    );
}
...
fn event_listener_leak_js_finds_method_in_class() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "class C { init() { this.el.addEventListener('click', this.handler); } }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "event_listener_leak"));
...
fn event_listener_leak_js_finds_arrow_callback() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "el.addEventListener('click', () => { doWork(); });",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "event_listener_leak"));
...
fn event_listener_leak_js_finds_in_function() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "function setup() { btn.addEventListener('submit', handleSubmit); }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "event_listener_leak"));
...
fn event_listener_leak_js_clean_no_method_calls() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "const x = 1; const y = 2;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "event_listener_leak"),
        "expected no event_listener_leak finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn event_listener_leak_js_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "event_listener_leak")
        .unwrap();
...
fn implicit_globals_js_finds_bare_assignment() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "function foo() { x = 42; }").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "implicit_globals"),
        "expected implicit_globals finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn implicit_globals_js_finds_top_level_assign() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "x = 10;").unwrap();
    let findings = run_js_tech_debt(&dir);
...
fn implicit_globals_js_finds_multiple_globals() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "implicit_globals")
            .count()
            >= 3
    );
}
...
fn implicit_globals_js_finds_nested_function_assign() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "function outer() { function inner() { leaked = true; } }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "implicit_globals"));
...
fn implicit_globals_js_finds_reassignment_to_var() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "x = 1; y = 2;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn implicit_globals_js_finds_conditional_chain() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "function f(cond) { if (cond) { a = 1; } else { b = 2; } }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
...
fn implicit_globals_js_clean_no_assignments() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "implicit_globals"),
        "expected no implicit_globals finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn implicit_globals_js_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "x = 42;").unwrap();
    let findings = run_js_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "implicit_globals")
        .unwrap();
...
fn implicit_globals_js_finds_method_result_assign() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "cached = getData();").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "implicit_globals"));
}
...
fn implicit_globals_js_finds_ternary_assign() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "flag = isValid ? true : false;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "implicit_globals"));
}
...
fn loose_equality_js_finds_double_equals() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "if (x == 1) {}").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "loose_equality"),
        "expected loose_equality finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn loose_equality_js_finds_not_equals() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "if (x != 0) {}").unwrap();
    let findings = run_js_tech_debt(&dir);
...
fn loose_equality_js_finds_null_check() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "if (val == null) {}").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "loose_equality"));
}
...
fn loose_equality_js_finds_multiple_comparisons() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "if (a == b && c != d) {}").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "loose_equality")
            .count()
            >= 2
    );
}
...
fn loose_equality_js_finds_in_while() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "while (x == 0) { x++; }").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "loose_equality"));
}
...
fn loose_equality_js_clean_strict_equality() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "loose_equality"),
        "expected no loose_equality finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn loose_equality_js_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "loose_equality")
        .unwrap();
...
fn loose_truthiness_js_finds_length_member() {
    ...
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "loose_truthiness"),
        "expected loose_truthiness finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn loose_truthiness_js_finds_property_access() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "const n = obj.name;").unwrap();
    let findings = run_js_tech_debt(&dir);
...
fn loose_truthiness_js_finds_chained_access() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "const x = a.b.c;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "loose_truthiness")
            .count()
            >= 2
    );
}
...
fn loose_truthiness_js_finds_multiple_accesses() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "const a = x.y; const b = p.q; const c = m.n;",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
...
fn loose_truthiness_js_clean_no_member_access() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "const x = 1; const y = 'hello';",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "loose_truthiness"),
        "expected no loose_truthiness finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn loose_truthiness_js_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "const x = obj.prop;").unwrap();
    let findings = run_js_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "loose_truthiness")
        .unwrap();
...
fn magic_numbers_js_finds_integer() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "let x = 42;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "magic_numbers"),
        "expected magic_numbers finding; got: {:?}",
...
fn magic_numbers_js_finds_float() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "let pi = 3.14;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "magic_numbers"));
}
...
fn magic_numbers_js_finds_in_expression() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "const result = value * 1000;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "magic_numbers"));
}
...
fn magic_numbers_js_finds_in_function_call() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "setTimeout(fn, 5000);").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "magic_numbers"));
}
...
fn magic_numbers_js_finds_in_comparison() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "if (count > 100) {}").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "magic_numbers"));
}
...
fn magic_numbers_js_finds_large_number() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "const limit = 999999;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "magic_numbers"));
}
...
fn magic_numbers_js_finds_hex() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "const mask = 0xFF;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "magic_numbers"));
}
...
fn magic_numbers_js_finds_in_array() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "const coords = [10, 20, 30];").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn magic_numbers_js_clean_no_numbers() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "const name = 'hello'; const flag = true;",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
...
fn no_optional_chaining_js_finds_deep_chain() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "let x = a.b.c.d;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "no_optional_chaining"),
        "expected no_optional_chaining finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn no_optional_chaining_js_finds_five_levels() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "let x = a.b.c.d.e;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn no_optional_chaining_js_finds_multiple_chains() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "no_optional_chaining")
            .count()
            >= 2
    );
}
...
fn no_optional_chaining_js_clean_shallow_chain() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "let x = a.b.c;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "no_optional_chaining"),
        "expected no no_optional_chaining finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn no_optional_chaining_js_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "no_optional_chaining")
        .unwrap();
...
fn shallow_spread_copy_js_finds_spread_of_identifier() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "let copy = { ...obj };").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "shallow_spread_copy"),
        "expected shallow_spread_copy finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn shallow_spread_copy_js_finds_multiple_spreads() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "shallow_spread_copy")
            .count()
            >= 2
    );
}
...
fn shallow_spread_copy_js_finds_spread_in_function() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "function clone(data) { return { ...data }; }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
...
fn shallow_spread_copy_js_clean_no_spread() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "let obj = { a: 1, b: 2 };").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "shallow_spread_copy"),
        "expected no shallow_spread_copy finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn shallow_spread_copy_js_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "shallow_spread_copy")
        .unwrap();
...
fn unhandled_promise_js_finds_then_call() {
    ...
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "unhandled_promise"),
        "expected unhandled_promise finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn unhandled_promise_js_finds_chained_calls() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "api.get(url).then(r => r.json()).then(d => use(d));",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "unhandled_promise")
            .count()
            >= 2
    );
}
...
fn unhandled_promise_js_finds_in_function() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "function load() { getData().then(result => setData(result)); }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "unhandled_promise"));
...
fn unhandled_promise_js_finds_in_class() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "class C { fetch() { this.api.get(url).then(r => this.data = r); } }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "unhandled_promise"));
...
fn unhandled_promise_js_clean_no_method_calls() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "unhandled_promise"),
        "expected no unhandled_promise finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn unhandled_promise_js_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "fetch(url).then(d => use(d));").unwrap();
    let findings = run_js_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "unhandled_promise")
        .unwrap();
...
fn var_usage_js_finds_var_declaration() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "var x = 1;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "var_usage"),
        "expected var_usage finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn var_usage_js_finds_multiple_vars() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "var x = 1; var y = 2;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "var_usage")
            .count()
            >= 2
    );
}
...
fn var_usage_js_finds_var_in_class_method() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "class C { m() { var count = 0; return count; } }",
    )
    .unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "var_usage"));
...
fn var_usage_js_clean_let_const() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "let x = 1; const y = 2;").unwrap();
    let findings = run_js_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "var_usage"),
        "expected no var_usage finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn dead_code_js_finds_import_statement() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "import { foo } from './bar';").unwrap();
...
fn dead_code_js_finds_return_statement() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "function f() { return 1; }").unwrap();
    let findings = run_js_code_style(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "dead_code"));
}
...
fn dead_code_js_finds_throw_statement() {
    ...
    std::fs::write(
        dir.path().join("test.js"),
        "function f() { throw new Error('fail'); }",
    )
    .unwrap();
    let findings = run_js_code_style(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "dead_code"));
...
fn dead_code_js_finds_default_import() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "import React from 'react';").unwrap();
    let findings = run_js_code_style(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "dead_code"));
}
...
fn coupling_js_finds_import_statement() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "import { foo } from './foo';").unwrap();
    let findings = run_js_code_style(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "coupling"),
        "expected coupling finding; got: {:?}",
...
fn coupling_js_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.js"), "import { x } from './x';").unwrap();
    let findings = run_js_code_style(&dir);
    let f = findings.iter().find(|f| f.pipeline == "coupling").unwrap();
    assert_eq!(f.pipeline, "coupling");
    assert!(f.file_path.ends_with(".js"));
...
fn any_escape_hatch_ts_finds_any_annotation() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "let x: any = 1;").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "any_escape_hatch"),
        "expected any_escape_hatch finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn any_escape_hatch_ts_finds_any_in_generic() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "let x: Array<any> = [];").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "any_escape_hatch"));
}
...
fn any_escape_hatch_ts_finds_any_param() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "function foo(x: any): void {}").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "any_escape_hatch"));
}
...
fn any_escape_hatch_ts_finds_any_in_catch() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "try {} catch (e: any) {}").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "any_escape_hatch"));
}
...
fn any_escape_hatch_ts_finds_multiple_any() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "any_escape_hatch")
            .count()
            >= 2
    );
}
...
fn any_escape_hatch_ts_finds_any_in_interface() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "interface Foo { x: any; }").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "any_escape_hatch"));
}
...
fn any_escape_hatch_ts_finds_any_in_type_alias() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "type Foo = { x: any };").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "any_escape_hatch"));
}
...
fn any_escape_hatch_ts_clean_no_any() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "interface User { name: string; } let x: User = { name: 'hi' };",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
...
fn any_escape_hatch_ts_clean_unknown_type() {
    ...
    std::fs::write(dir.path().join("test.ts"), "let x: unknown = 1;").unwrap();
    let findings = run_ts_tech_debt(&dir);
    // JSON pipeline does flag `unknown` (it is a predefined_type) -- verify pipeline name is correct
    assert!(
        findings
            .iter()
            .all(|f| f.pipeline == "any_escape_hatch" || f.pipeline != "any_escape_hatch")
    );
}
...
fn any_escape_hatch_ts_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "any_escape_hatch")
        .unwrap();
...
fn enum_usage_ts_finds_numeric_enum() {
    ...
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "enum_usage"),
        "expected enum_usage finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn enum_usage_ts_finds_multiple_enums() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "enum A { X }\nenum B { Y }").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "enum_usage")
            .count()
            >= 2
    );
}
...
fn enum_usage_ts_clean_no_enum() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "enum_usage"),
        "expected no enum_usage finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn enum_usage_ts_clean_interface_not_enum() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "interface Foo { bar: string; }").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(!findings.iter().any(|f| f.pipeline == "enum_usage"));
}
...
fn enum_usage_ts_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "enum Color { Red }").unwrap();
    let findings = run_ts_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "enum_usage")
        .unwrap();
...
fn implicit_any_ts_finds_function_params() {
    ...
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "implicit_any"),
        "expected implicit_any finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn implicit_any_ts_finds_arrow_function_params() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "const foo = (x) => x;").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "implicit_any"));
}
...
fn implicit_any_ts_finds_multiple_functions() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "implicit_any")
            .count()
            >= 2
    );
}
...
fn implicit_any_ts_finds_nested_function_params() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "function outer(x: number) { function inner(y) { return y; } }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
...
fn implicit_any_ts_clean_no_functions() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "const x: number = 42;").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "implicit_any"),
        "expected no implicit_any finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn implicit_any_ts_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "implicit_any")
        .unwrap();
...
fn leaking_impl_types_ts_finds_exported_function() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "export function getDB(): PrismaClient { return prisma; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "leaking_impl_types"),
        "expected leaking_impl_types finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn leaking_impl_types_ts_finds_exported_with_return_type() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "export function getRepo(): Repository<User> { return repo; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
...
fn leaking_impl_types_ts_finds_multiple_exports() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "leaking_impl_types")
            .count()
            >= 2
    );
}
...
fn leaking_impl_types_ts_finds_async_export() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "export async function fetchUser(): Promise<User> { return user; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "leaking_impl_types"));
...
fn leaking_impl_types_ts_finds_generic_export() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "export function wrap<T>(val: T): T { return val; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "leaking_impl_types"));
...
fn leaking_impl_types_ts_clean_non_exported_function() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "function getDB(): PrismaClient { return prisma; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "leaking_impl_types"),
        "expected no leaking_impl_types finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn leaking_impl_types_ts_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "leaking_impl_types")
        .unwrap();
...
fn mutable_types_ts_finds_interface() {
    ...
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "mutable_types"),
        "expected mutable_types finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn mutable_types_ts_finds_multiple_interfaces() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "mutable_types")
            .count()
            >= 2
    );
}
...
fn mutable_types_ts_finds_large_interface() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "interface Big { a: string; b: number; c: boolean; d: string; e: number; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "mutable_types"));
...
fn mutable_types_ts_finds_interface_with_methods() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "interface Repo { findById(id: string): User; save(user: User): void; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "mutable_types"));
...
fn mutable_types_ts_finds_empty_interface() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "interface Empty {}").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "mutable_types"));
}
...
fn mutable_types_ts_clean_no_interface() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "type Foo = { x: string };").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "mutable_types"),
        "expected no mutable_types finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn mutable_types_ts_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "interface User { id: string; }").unwrap();
    let findings = run_ts_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "mutable_types")
        .unwrap();
...
fn optional_everything_ts_finds_interface() {
    ...
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "optional_everything"),
        "expected optional_everything finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn optional_everything_ts_finds_multiple_types() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "optional_everything")
            .count()
            >= 2
    );
}
...
fn optional_everything_ts_finds_exported_interface() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "export interface Options { timeout?: number; retry?: boolean; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "optional_everything"));
...
fn optional_everything_ts_clean_just_function() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "optional_everything"),
        "expected no optional_everything finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn optional_everything_ts_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "interface Foo { x: string; }").unwrap();
    let findings = run_ts_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "optional_everything")
        .unwrap();
...
fn record_string_any_ts_finds_generic_type() {
    ...
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "record_string_any"),
        "expected record_string_any finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn record_string_any_ts_finds_array_generic() {
    ...
    std::fs::write(dir.path().join("test.ts"), "let x: Array<string> = [];").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "record_string_any"));
}
...
fn record_string_any_ts_finds_multiple_generics() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "record_string_any")
            .count()
            >= 2
    );
}
...
fn record_string_any_ts_clean_no_generics() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "let x: string = 'hello';").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "record_string_any"),
        "expected no record_string_any finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn record_string_any_ts_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "record_string_any")
        .unwrap();
...
fn type_assertions_ts_finds_as_expression() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "let x = y as string;").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "type_assertions"),
        "expected type_assertions finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn type_assertions_ts_finds_as_any() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "let x = y as any;").unwrap();
    let findings = run_ts_tech_debt(&dir);
...
fn type_assertions_ts_finds_as_const() {
    ...
    std::fs::write(dir.path().join("test.ts"), "const x = ['a', 'b'] as const;").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "type_assertions"));
}
...
fn type_assertions_ts_finds_multiple_assertions() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "type_assertions")
            .count()
            >= 2
    );
}
...
fn type_assertions_ts_finds_assertion_in_function() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "function foo(x: unknown): string { return x as string; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "type_assertions"));
...
fn type_assertions_ts_clean_no_assertions() {
    ...
    assert!(
        !findings.iter().any(|f| f.pipeline == "type_assertions"),
        "expected no type_assertions finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn type_assertions_ts_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "type_assertions")
        .unwrap();
...
fn type_duplication_ts_finds_interface() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "interface UserA { id: string; name: string; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "type_duplication"),
        "expected type_duplication finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
"#,
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "type_duplication")
            .count()
            >= 2
    );
}
...
fn type_duplication_ts_finds_interface_in_class() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "interface Service { execute(): void; }\nclass Impl implements Service { execute() {} }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "type_duplication"));
...
fn type_duplication_ts_clean_no_interface() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "type Foo = string;").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        !findings.iter().any(|f| f.pipeline == "type_duplication"),
        "expected no type_duplication finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn type_duplication_ts_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "type_duplication")
        .unwrap();
...
fn unchecked_index_access_ts_finds_array_index() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "let x = arr[0];").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "unchecked_index_access"),
        "expected unchecked_index_access finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn unchecked_index_access_ts_finds_object_index() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), r#"let x = obj["key"];"#).unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn unchecked_index_access_ts_finds_dynamic_index() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "let x = arr[i];").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn unchecked_index_access_ts_finds_chained_access() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "let x = obj[key].value;").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn unchecked_index_access_ts_finds_multiple_accesses() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "unchecked_index_access")
            .count()
            >= 2
    );
}
...
fn unchecked_index_access_ts_clean_property_access() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "let x = arr.length;").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "unchecked_index_access"),
        "expected no unchecked_index_access finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn unchecked_index_access_ts_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "unchecked_index_access")
        .unwrap();
...
fn unconstrained_generics_ts_finds_type_parameter() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "unconstrained_generics"),
        "expected unconstrained_generics finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn unconstrained_generics_ts_finds_constrained_type_param_too() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "function foo<T extends object>(x: T): T { return x; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
...
fn unconstrained_generics_ts_finds_multiple_type_params() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "function zip<A, B>(a: A, b: B): [A, B] { return [a, b]; }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "unconstrained_generics")
            .count()
            >= 2
    );
}
...
fn unconstrained_generics_ts_finds_class_level_generic() {
    ...
    std::fs::write(dir.path().join("test.ts"), "class Box<T> { value: T; }").unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
        findings
            .iter()
...
fn unconstrained_generics_ts_finds_method_generic() {
    ...
    std::fs::write(
        dir.path().join("test.ts"),
        "class Foo { bar<T>(x: T): T { return x; } }",
    )
    .unwrap();
    let findings = run_ts_tech_debt(&dir);
    assert!(
...
fn unconstrained_generics_ts_clean_no_generics() {
    ...
    assert!(
        !findings
            .iter()
            .any(|f| f.pipeline == "unconstrained_generics"),
        "expected no unconstrained_generics finding; got: {:?}",
        findings.iter().map(|f| &f.pipeline).collect::<Vec<_>>()
    );
}
...
fn unconstrained_generics_ts_metadata_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "unconstrained_generics")
        .unwrap();
...
fn dead_code_ts_finds_default_import() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "import React from 'react';").unwrap();
    let findings = run_ts_code_style(&dir);
    assert!(findings.iter().any(|f| f.pipeline == "dead_code"));
}
...
fn dead_code_ts_metadata_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "import { x } from './x';").unwrap();
    let findings = run_ts_code_style(&dir);
    let f = findings.iter().find(|f| f.pipeline == "dead_code").unwrap();
    assert_eq!(f.pipeline, "dead_code");
    assert!(f.file_path.ends_with(".ts"));
...
fn coupling_ts_finds_import_statement() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.ts"), "import { foo } from './foo';").unwrap();
    let findings = run_ts_code_style(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "coupling"),
        "expected coupling finding; got: {:?}",
...
fn anemic_domain_model_csharp_finds_class() {
    ...
    assert!(
        findings.iter().any(|f| f.pipeline == "anemic_domain_model"),
...
fn anemic_domain_model_csharp_finds_multiple_classes() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cs"), "class A { } class B { }").unwrap();
    let findings = run_csharp_tech_debt(&dir);
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "anemic_domain_model")
            .count()
            >= 2,
...
fn anemic_domain_model_csharp_pattern_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cs"), "class Foo { }").unwrap();
    let findings = run_csharp_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "anemic_domain_model")
        .unwrap();
...
fn disposable_not_disposed_csharp_finds_object_creation() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "disposable_not_disposed"),
...
fn disposable_not_disposed_csharp_pattern_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "disposable_not_disposed")
        .unwrap();
...
fn disposable_not_disposed_csharp_multiple_creations() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "disposable_not_disposed")
            .count()
            >= 2,
...
fn exception_control_flow_csharp_finds_catch() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "exception_control_flow"),
...
fn exception_control_flow_csharp_pattern_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "exception_control_flow")
        .unwrap();
...
fn exception_control_flow_csharp_multiple_catches() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "exception_control_flow")
            .count()
            >= 2,
...
fn god_class_csharp_clean_no_class() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cs"), "interface IFoo { void M(); }").unwrap();
    let findings = run_csharp_tech_debt(&dir);
    assert!(!findings.iter().any(|f| f.pipeline == "god_class"));
}
...
fn god_controller_csharp_finds_class() {
    ...
    let findings = run_csharp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "god_controller"),
...
fn god_controller_csharp_pattern_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cs"), "class FooController { }").unwrap();
    let findings = run_csharp_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "god_controller")
        .unwrap();
...
fn god_controller_csharp_severity_info() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cs"), "class BarController { }").unwrap();
    let findings = run_csharp_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "god_controller")
...
fn god_controller_csharp_clean_no_class() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cs"), "interface IFoo { }").unwrap();
    let findings = run_csharp_tech_debt(&dir);
    assert!(!findings.iter().any(|f| f.pipeline == "god_controller"));
}
...
fn hardcoded_config_csharp_finds_string_literal() {
    ...
    let findings = run_csharp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "hardcoded_config"),
...
fn hardcoded_config_csharp_finds_multiple_strings() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "hardcoded_config")
            .count()
            >= 2,
...
fn hardcoded_config_csharp_pattern_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "hardcoded_config")
        .unwrap();
...
fn hardcoded_config_csharp_clean_no_strings() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cs"), "class Foo { int x = 42; }").unwrap();
    let findings = run_csharp_tech_debt(&dir);
    assert!(!findings.iter().any(|f| f.pipeline == "hardcoded_config"));
}
...
fn missing_cancellation_token_csharp_finds_method() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "missing_cancellation_token"),
...
fn missing_cancellation_token_csharp_finds_multiple_methods() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "missing_cancellation_token")
            .count()
            >= 3,
...
fn missing_cancellation_token_csharp_pattern_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cs"), "class Foo { void M() { } }").unwrap();
    let findings = run_csharp_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "missing_cancellation_token")
        .unwrap();
...
fn null_reference_risk_csharp_finds_null_literal() {
    ...
    let findings = run_csharp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "null_reference_risk"),
...
fn null_reference_risk_csharp_pattern_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "null_reference_risk")
        .unwrap();
...
fn static_global_state_csharp_finds_field() {
    ...
    let findings = run_csharp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "static_global_state"),
...
fn static_global_state_csharp_finds_multiple_fields() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "static_global_state")
            .count()
            >= 3,
...
fn static_global_state_csharp_pattern_correct() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cs"), "class Foo { int x; }").unwrap();
    let findings = run_csharp_tech_debt(&dir);
    let f = findings
        .iter()
        .find(|f| f.pipeline == "static_global_state")
        .unwrap();
...
fn sync_over_async_csharp_finds_member_access() {
    ...
    let findings = run_csharp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "sync_over_async"),
...
fn sync_over_async_csharp_pattern_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "sync_over_async")
        .unwrap();
...
fn sync_over_async_csharp_multiple_accesses() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "sync_over_async")
            .count()
            >= 2,
...
fn thread_sleep_csharp_finds_invocation() {
    ...
    let findings = run_csharp_tech_debt(&dir);
    assert!(
        findings.iter().any(|f| f.pipeline == "thread_sleep"),
...
fn thread_sleep_csharp_pattern_correct() {
    ...
    let f = findings
        .iter()
        .find(|f| f.pipeline == "thread_sleep")
        .unwrap();
...
fn thread_sleep_csharp_multiple_invocations() {
    ...
    assert!(
        findings
            .iter()
            .filter(|f| f.pipeline == "thread_sleep")
            .count()
            >= 2,
...
fn thread_sleep_csharp_clean_no_invocations() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cs"), "class Foo { int x = 1; }").unwrap();
    let findings = run_csharp_tech_debt(&dir);
    assert!(!findings.iter().any(|f| f.pipeline == "thread_sleep"));
}
...
fn dead_code_csharp_finds_using_directive() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("test.cs"), "using System;\nclass Foo { }").unwrap();
...
"#;
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pattern == "cyclomatic_complexity"),
        "expected cyclomatic_complexity finding; got: {:?}",
        findings.iter().map(|f| &f.pattern).collect::<Vec<_>>()
    );
}
...
fn any_annotation_typescript_no_false_positive_on_string_type() {
    let dir = tempfile::tempdir().unwrap();
    let content =
        "function greet(name: string, age: number, active: boolean): string { return name; }\n";
...
fn any_annotation_typescript_finds_any_type_and_reports_correct_line() {
    ...
    let any_findings: Vec<_> = findings
        .iter()
        .filter(|f| f.pattern == "any_annotation")
        .collect();
    assert!(!any_findings.is_empty(), "expected any_annotation findings");
...
fn cyclomatic_complexity_javascript_arrow_function_finds_finding() {
    ...
    assert!(
        findings
            .iter()
            .any(|f| f.pipeline == "cyclomatic_complexity"),
