...
fn build_block(
    ...
    for child in block_node.children(&mut cursor) {
        match child.kind() {
            // ── branching ───────────────────────────────────────────
            "if_statement" => {
                match build_if(cfg, current, &child, source) {
            ...
            // ── switch ──────────────────────────────────────────────
            "switch_statement" => {
                match build_switch(cfg, current, &child, source) {
                    Some(join) => current = join,
                    None => return None,
                }
            }
            ...
            "compound_statement" => {
                match build_block(cfg, current, &child, source) {
...
fn build_if(
    ...
    cfg.blocks.add_edge(current, true_block, CfgEdge::TrueBranch);
        ...
        Some(alt) => {
            if alt.kind() == "else_clause" {
                ...
                match body {
                    ...
                    Some(b) if b.kind() == "if_statement" => {
                        build_if(cfg, false_block, &b, source)
                    }
                    Some(b) => {
    ...
    for case in body.children(&mut cursor) {
        ...
        for stmt in case.children(&mut case_cursor) {
            match stmt.kind() {
                ...
                "compound_statement" => {
                    match build_block(cfg, case_current, &stmt, source) {
    ...
    if let Some(init) = node.child_by_field_name("declarator") {
        if init.kind() == "init_declarator" {
            ...
            if let Some(val) = value {
                if let Some(resource_call) = detect_c_resource_acquire(&val, source) {
    ...
    match expr.kind() {
        ...
        "assignment_expression" => {
            ...
            if let Some(ref r) = right {
                if let Some(resource_call) = detect_c_resource_acquire(r, source) {
                    cfg.blocks[block].statements.push(CfgStatement {
                        kind: CfgStatementKind::ResourceAcquire {
                            target,
                            resource_type: resource_call,
