...
use super::go_primitives::{compile_numeric_literal_query, find_capture_index};

const EXCLUDED_VALUES: &[&str] = &["0", "1", "2", "0.0", "1.0"];

const EXEMPT_ANCESTOR_KINDS: &[&str] = &["const_declaration", "const_spec"];
...
impl GoMagicNumbersPipeline {
    ...
    fn is_exempt_context(node: tree_sitter::Node) -> bool {
        ...
        if let Some(parent) = node.parent() {
            if parent.kind() == "index_expression" {
