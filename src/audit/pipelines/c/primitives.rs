...
/// Forward declarations like `struct Foo;` or `void func(void);` have no
/// `compound_statement` child (no function body) and no `init_declarator` child
/// (no variable initializer). Both are definitive signs of a real definition.
pub fn is_c_forward_declaration(decl: tree_sitter::Node) -> bool {
...
/// Check if a C source file should be treated as generated (not human-authored).
/// Checks known generated-file filename patterns first, then scans the first
/// 10 lines of source for common generated-file marker phrases.
pub fn is_generated_c_file(file_path: &str, source: &[u8]) -> bool {
    let file_name = std::path::Path::new(file_path)
        .file_name()
        .and_then(|f| f.to_str())
        .unwrap_or("");

    if file_name == "lex.yy.c" || file_name == "lex.yy.h" {
        return true;
    }
    if file_name.ends_with(".tab.c") || file_name.ends_with(".tab.h") {
        return true;
    }
    if file_name.starts_with("moc_") {
        return true;
    }
...
mod tests {
    ...
    #[test]
    fn is_c_forward_declaration_struct_decl() {
        // In tree-sitter C, `struct Foo;` parses as struct_specifier (not declaration).
        // is_c_forward_declaration targets declaration nodes; struct forward declarations
        // are identified by the absence of a body field on the struct_specifier.
        let src = "struct Foo;";
        let (tree, _source) = parse_c(src);
        let node = tree.root_node().named_child(0).unwrap();
        assert_eq!(node.kind(), "struct_specifier");
        assert!(node.child_by_field_name("body").is_none());
    }

    #[test]
    fn is_c_forward_declaration_function_prototype() {
        let src = "void foo(void);";
        let (tree, _source) = parse_c(src);
        let decl = tree.root_node().named_child(0).unwrap();
        assert_eq!(decl.kind(), "declaration");
        assert!(is_c_forward_declaration(decl));
    }

    #[test]
    fn is_c_forward_declaration_function_definition_is_not() {
        let src = "void foo(void) {}";
        let (tree, _source) = parse_c(src);
        let func = tree.root_node().named_child(0).unwrap();
        assert_eq!(func.kind(), "function_definition");
        assert!(!is_c_forward_declaration(func));
    }

    #[test]
    fn is_c_forward_declaration_var_with_initializer_is_not() {
        let src = "int x = 0;";
        let (tree, _source) = parse_c(src);
        let decl = tree.root_node().named_child(0).unwrap();
        assert_eq!(decl.kind(), "declaration");
    ...
    #[test]
    fn is_generated_c_file_lex_yy_c() {
        assert!(is_generated_c_file("lex.yy.c", b"void yylex(void) {}"));
    }

    #[test]
    fn is_generated_c_file_tab_c() {
        assert!(is_generated_c_file("parser.tab.c", b"int yyparse(void) { return 0; }"));
    }

    #[test]
    fn is_generated_c_file_moc() {
        assert!(is_generated_c_file("moc_widget.c", b"void setup() {}"));
    }

    #[test]
    fn is_generated_c_file_by_comment() {
        let src = b"/* Generated by bison 3.8.2 */\nint yyparse(void) { return 0; }";
        assert!(is_generated_c_file("myparser.c", src));
    }

    #[test]
    fn is_generated_c_file_do_not_edit() {
        let src = "// do not edit \u{2014} generated by protoc\nvoid pb_init(void) {}";
        assert!(is_generated_c_file("proto.c", src.as_bytes()));
    }

    #[test]
    fn is_generated_c_file_normal_file_is_not() {
        let src = b"void my_function(void) { return; }";
        assert!(!is_generated_c_file("mymodule.c", src));
    }
}
