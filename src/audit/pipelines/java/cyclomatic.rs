...
impl Pipeline for CyclomaticComplexityPipeline {
    ...
    fn description(&self) -> &str {
        "Detects methods with high cyclomatic complexity (>10)"
    }
                ...
                (name_node, body_node, method_node)
            {
                ...
                if cc > CC_THRESHOLD {
                    ...
                    findings.push(AuditFinding {
                        ...
                        message: format!(
                            "Cyclomatic complexity of {cc} (threshold: {CC_THRESHOLD}) in method '{name}'"
                        ),
...
mod tests {
    ...
    fn detects_high_cyclomatic_complexity() {
        // Method with many branches: 1 base + 11 if statements = CC 12
        let src = r#"
class Foo {
    void complex(int x) {
        if (x == 1) {}
