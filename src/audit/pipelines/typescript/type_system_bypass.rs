...
impl Pipeline for TypeSystemBypassPipeline {
    ...
    fn check(&self, tree: &Tree, source: &[u8], file_path: &str) -> Vec<AuditFinding> {
        ...
        // as expression on untrusted data + double cast detection
        {
            ...
            let mut matches =
                cursor.matches(&self.as_expr_query, tree.root_node(), source);
            ...
            while let Some(m) = matches.next() {
                ...
                if let Some(as_expr) = as_node {
                    ...
                    let inner = as_expr.child_by_field_name("expression")
                    ...
                    if let Some(inner_node) = inner {
                        if inner_node.kind() == "as_expression" {
                            ...
                            findings.push(AuditFinding {
                                ...
                                message: "Double type assertion (`as X as Y`) — bypasses type safety".to_string(),
                        ...
                        let is_untrusted = UNTRUSTED_SOURCES
                            .iter()
                            .any(|s| inner_text.contains(s));
        ...
        // Non-null assertion on untrusted data
        {
            let mut cursor = QueryCursor::new();
            let mut matches =
                cursor.matches(&self.non_null_query, tree.root_node(), source);
