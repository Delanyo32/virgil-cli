...
"#;
        Ok(Self {
            method_call_query: Arc::new(
                Query::new(&ts_lang, method_call_str)
                    .with_context(|| "failed to compile method_call query for TS n_plus_one")?,
            ),
            direct_call_query: Arc::new(
                Query::new(&ts_lang, direct_call_str)
                    .with_context(|| "failed to compile direct_call query for TS n_plus_one")?,
            ),
        })
    }
    ...
    /// Returns true if `node` is inside a loop-like context.
...
items.forEach(async (item: Item) => {
    ...
    fn ignores_call_outside_loop() {
        let src = "\
const user: User = await User.findOne({ id: 1 });
...
for (const item of items) {
    ...
    #[test]
    fn tsx_compiles() {
        NPlusOneQueriesPipeline::new(Language::Tsx).unwrap();
    }
}
