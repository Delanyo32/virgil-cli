...
use super::primitives::{extract_snippet, has_modifier, is_csharp_suppressed, is_generated_code};
...
impl Pipeline for ModuleSizeDistributionPipeline {
    ...
    fn check(&self, tree: &Tree, source: &[u8], file_path: &str) -> Vec<AuditFinding> {
        ...
        if is_csharp_suppressed(source, root, "module_size_distribution") {
            return findings;
        }
...
"#;
    ...
    fn test_file_not_analyzed() {
        ...
        src.push_str("}\n");
        let findings = parse_and_check_file(&src, "OrderRepositoryTests.cs");
        assert!(findings.is_empty(), "test files must not produce findings");
    }

    #[test]
    fn test_designer_file_not_analyzed() {
        let mut src = String::from("namespace MyApp {\n");
        for i in 0..31 {
            src.push_str(&format!("public class Class_{} {{ }}\n", i));
        }
        ...
        assert!(findings.is_empty(), "Designer.cs files must not produce findings");
    }

    #[test]
    fn test_auto_generated_header_suppresses() {
        let mut src = String::from("// <auto-generated />\nnamespace MyApp {\n");
        for i in 0..35 {
            src.push_str(&format!("public class Class_{} {{ }}\n", i));
        }
        src.push_str("}\n");
        let findings = parse_and_check_file(&src, "Generated.cs");
        assert!(findings.is_empty(), "auto-generated files must not produce findings");
    }
}
