...
1. **Search for the symbol:**
   ```bash
   virgil projects search <SYMBOL_NAME> --data-dir <DATA> --format json
   ```
   Note the file_path and line number of the definition.

2. **Read the definition:**
   ```bash
   virgil projects read <FILE_PATH> --start-line <N> --end-line <M> --data-dir <DATA> --root <PROJECT>
   ```

3. **Find all callers** (who imports this symbol):
   ```bash
   virgil projects callers <SYMBOL_NAME> --data-dir <DATA> --format json
   ```

4. **Find all dependents** of the file containing the symbol:
   ```bash
   virgil projects dependents <FILE_PATH> --data-dir <DATA> --format json
   ```

5. **Read key callers** to understand usage patterns:
   ```bash
   virgil projects read <CALLER_FILE> --start-line <N> --end-line <M> --data-dir <DATA> --root <PROJECT>
   ```
...
1. **Parse the codebase:**
   ```bash
   virgil projects parse <PROJECT_DIR> --output <DATA>
   ```
...
4. **Find entry points** (most depended-on files):
   ```bash
   virgil projects files --sort dependents --limit 10 --data-dir <DATA> --format json
   ```

5. **Outline entry points:**
   ```bash
   virgil projects outline <ENTRY_FILE> --data-dir <DATA> --format json
   ```

6. **Read key files:**
   ```bash
   virgil projects read <ENTRY_FILE> --data-dir <DATA> --root <PROJECT_DIR>
   ```

7. **Understand the dependency graph:**
   ```bash
   virgil projects deps <ENTRY_FILE> --data-dir <DATA> --format json
   ```

8. **Check external dependencies:**
   ```bash
   virgil projects imports --external --limit 50 --data-dir <DATA> --format json
   ```

9. **Read doc comments for orientation:**
   ```bash
   virgil projects comments --kind doc --limit 30 --data-dir <DATA> --format json
   ```
...
1. **Search for related symbols:**
   ```bash
   virgil projects search <BUG_RELATED_TERM> --data-dir <DATA> --format json
   ```

2. **Read the suspect code:**
   ```bash
   virgil projects read <FILE> --start-line <N> --end-line <M> --data-dir <DATA> --root <PROJECT>
   ```

3. **Check what the suspect file depends on:**
   ```bash
   virgil projects deps <FILE> --data-dir <DATA> --format json
   ```

4. **Find who calls the suspect symbol:**
   ```bash
   virgil projects callers <SYMBOL> --data-dir <DATA> --format json
   ```

5. **Read the callers** to understand how the buggy code is invoked:
   ```bash
   virgil projects read <CALLER_FILE> --data-dir <DATA> --root <PROJECT>
   ```

6. **Check comments near the bug** for context or known issues:
   ```bash
   virgil projects comments --file <FILE_PREFIX> --data-dir <DATA> --format json
   ```

7. **Search for TODO/FIXME near the area:**
   ```bash
   virgil projects query "SELECT file_path, text, start_line FROM comments
...
2. **Check who depends on that file:**
   ```bash
   virgil projects dependents <FILE> --data-dir <DATA> --format json
   ```

3. **List all external dependencies:**
   ```bash
   virgil projects imports --external --data-dir <DATA> --format json
   ```

4. **List all internal imports:**
   ```bash
   virgil projects imports --internal --data-dir <DATA> --format json
   ```

5. **Find most-imported external packages:**
   ```bash
   virgil projects query "SELECT module_specifier, COUNT(*) AS cnt
...
6. **Find orphan files** (imported by nothing):
   ```bash
   virgil projects query "SELECT f.path FROM files f
...
7. **Detect potential circular dependencies:**
   ```bash
   virgil projects query "SELECT a.source_file AS file_a, b.source_file AS file_b
...
2. **Search for exported functions:**
   ```bash
   virgil projects search "" --kind function --exported --data-dir <DATA> --format json
   ```

3. **Search for exported classes/interfaces:**
   ```bash
   virgil projects search "" --kind class --exported --data-dir <DATA> --format json
   virgil projects search "" --kind interface --exported --data-dir <DATA> --format json
   ```

4. **Find all exported types via SQL:**
   ```bash
   virgil projects query "SELECT name, kind, file_path, start_line
...
5. **Find re-exports** (barrel files):
   ```bash
   virgil projects imports --kind re_export --data-dir <DATA> --format json
   ```

6. **Count exported symbols per file** to find API-dense modules:
   ```bash
   virgil projects query "SELECT file_path, COUNT(*) AS exports
   FROM symbols WHERE is_exported = true
...
7. **Check documentation coverage** of the API surface:
   ```bash
   virgil projects query "SELECT s.name, s.kind, s.file_path,
